...
export const ref = (model, required = true) => ({ type: mongoose.Schema.Types.ObjectId, ref: model, required });
export const money = (required = true) => ({ type: Number, required, min: 0, max: Number.MAX_SAFE_INTEGER, validate: Number.isSafeInteger });
export const quantity = { type: Number, required: true, min: 0, validate: Number.isFinite };
export const state = (values, initial) => ({ type: String, enum: values.split(' '), default: initial, required: true });
